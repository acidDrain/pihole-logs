...
import { sToMs, fifteenMinutes } from './util';
...
export default async ({ piholeAddress, verifySSL = false, PIHOLE_API_TOKEN, startTime = (NOW - fifteenMinutes), endTime = NOW }: IPiholeMain) => {
  ...
  return { data: data.map(formatRecord) };
